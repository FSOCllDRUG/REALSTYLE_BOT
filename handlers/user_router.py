...
async def cancel_fsm(message: Message, state: FSMContext):
    ...
    await message.answer("Действие отменено\n\n"
                         "Вернул тебя в главное меню", reply_markup=await inline_main(message.from_user.id in
                                                                                      env_admins))
...
async def cmd_start(message: Message):
    await message.answer(f"Привет, {message.from_user.full_name}!", reply_markup=await reply_menu())
...
async def main_menu_inline(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer("Ты в главном меню!",
                                  reply_markup=await inline_main(callback.from_user.id in env_admins))


@user_router.message(F.text == "Меню")
async def main_menu_reply(message: Message, state: FSMContext):
    await state.clear()
    await message.answer("Ты в главном меню!",
                         reply_markup=await inline_main(message.from_user.id in env_admins))
